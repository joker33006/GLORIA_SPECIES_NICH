...
nich <- nich[code_ref,on=.(code=GBIF_code),all=TRUE]#將所有氣候區位資料結合
nich[,i.V1:=NULL]
...
dif[,h_p13:=heat_ind_P3-heat_ind_P1][ 
        ,p_p13:=pre_ind_P3-pre_ind_P1]
t.test_total_r <- NULL
JNJ_exc <- dif[Summit=="JNJ"&Summit_dir!="E10"]
t.test(JNJ_exc[,h_p23],mu=0)
for (j in c('h_p12','h_p23','p_p12','p_p23','h_p13',"p_p13")){
...
t.test_total_r[is.na(sign),Sign:=""]
t_r <- dcast(data=t.test_total_r,Summit~variable,value.var ="sign")
t.test_total_r[,sign]
t_r <- t_r[dif[,lapply(.SD,max),by=Summit,.SDcols=11:16],on=.(Summit=Summit)]
t_r[,Summit:=factor(Summit,levels= c('SEN','YAT','SUN','TSW','DSH',"JNJ"))]
############
w=0.4
for (i in 1:4){
var <- c("h_p12","h_p23","p_p12","p_p23","h_p13","p_p13")
ind <- c("Thermophilization indicator","moist-philization indicator")
pos <- c(1,60)
ggplot(dif,aes(x=Summit,y=get(var[i]),fill=Summit))+
   geom_boxplot(width=w,alpha = 0.5)+
...
ggplot(dif,aes(x=Summit,y=get(var[i]),fill=Summit))+
   geom_boxplot(width=w,alpha = 0.5)+
  geom_text(data=t_r, aes(x=Summit,y=get(paste0("i.",var[i]))+pos[i], label=get(var[i])))+
   labs(x="Summit",y=ind[i],title=paste0("(",letters[i],")"))+
  theme(legend.position = "None")+
  scale_x_discrete(limits= c('SEN','YAT','SUN','TSW','DSH',"JNJ"))+
  geom_hline(yintercept=0,linetype="dotted",colour="#BB0000")
ggsave(paste0(svpath,"/plot/",ind[i],"_",var[i],".jpeg"),width = 4,height = 3,dpi=600)
}
...
spe_den <-unique(cov_ch[,.(region,Name,code,bio_1_3,bio_12_3,bio_1_r,bio_12_r)])

############### climate niche distribution plot 
nich_pre <- ggplot(spe_den, aes(x = region, y = bio_12_3,fill=region,color=region)) + 
  stat_halfeye(
    adjust = .5, 
    width = .5, 
    .width = 0,
  ...
  labs(x="Region",y="Precipitation niche (mm)",title="(b)")+
  theme(legend.position = "none")
nich_temp <- ggplot(spe_den, aes(x = region, y = bio_1_3,fill=region,color=region)) + 
   stat_halfeye(
    adjust = .5, 
    width = .5, 
    .width = 0,
  ...
  labs(x="Region",y="Temperature niche (°C)",title="(a)")+
  theme(legend.position = "none")
nich_temp+nich_pre
ggsave(paste0(svpath,"/plot/climate_niche_all_species.jpeg"),width = 8,height = 5,dpi=600)
###########################finish the plot
###################glm of the cover and species climate niche
?glm
m1 <- glm(cov12_r~bio_1_3+bio_12_3,
          data=cov_ch[cov12_r!=1],family=Gamma(link = "log"))
m2 <- glm(cov23_r~bio_1_3+bio_12_3,
          data=cov_ch[cov23_r!=1],family=Gamma(link = "log"))

tr <- rbind(data.table(cov="cov_12",
                            coef=rownames(summary(m1)$coefficients), 
                            summary(m1)$coefficients),
                 data.table(cov="cov_23",
                            coef=rownames(summary(m2)$coefficients), 
                            summary(m2)$coefficients))

fwrite(tr,paste0(svpath,"/glm_test_clim_nich_cover.csv"))
##################to test the each summit
summit_tr <- NULL
for (i in c("SEN","YAT","SUN","TSW","DSH","JNJ")){
  m3 <- glm(cov23_r~bio_1_3+bio_12_3,
          data=cov_ch[cov23_r!=1&Summit==`i`],family=Gamma(link = "log"))
  m4 <- glm(cov23_r~bio_1_3+bio_12_3,
          data=cov_ch[cov23_r!=1&Summit==`i`],family=Gamma(link = "log"))
tr_2 <- rbind(data.table(summit=i,cov="cov_12",
                            coef=rownames(summary(m3)$coefficients), 
                            summary(m3)$coefficients),
                 data.table(summit=i,cov="cov_23",
                            coef=rownames(summary(m4)$coefficients), 
                            summary(m4)$coefficients))
summit_tr <- rbind(summit_tr,tr_2)
}
fwrite(summit_tr,paste0(svpath,"/glm_test_each_clim_nich_cover.csv"))
############mc1 to mc2
for (i in c("DAS","SYU")){
p <- ggplot(cov_ch[region==`i`&!cov_12==0],aes(x=bio_1_3,y=bio_12_3))+
  geom_point(aes(color = cag_12, size = cov_12), alpha = 0.5,na.rm = TRUE)+
  facet_grid(~Summit)+
  labs(x="",
       y="",
       color="Alter",size="Cover difference")+
  scale_size(breaks=seq(20,180,40))+
  scale_color_discrete(direction=-1)
if (i=="SYU"){p <- p+
  theme(legend.position = "none")}
assign(paste0("pr_",i),p)
}
 
pr <- patchworkGrob(pr_DAS+pr_SYU+ plot_layout(ncol = 1, guides = "collect"))
pr <- gridExtra::grid.arrange(pr, 
                              left ="Precipitation niche (mm)",
                              bottom = textGrob("Temperature niche (°C)", 
                                                vjust = -1,
                                                hjust=0.7),
                              top= textGrob("(a) MC1 to MC2", hjust = 2.5))  
ggsave(paste0(svpath,"/plot/cover_vs_nich_MC12.jpeg"),plot=pr,width=8,height=5,dpi=600)


############mc2 to mc3
for (i in c("DAS","SYU")){
p <- ggplot(cov_ch[region==`i`&!cov_23==0],aes(x=bio_1_3,y=bio_12_3))+
  geom_point(aes(color = cag_23, size = cov_23), alpha = 0.5,na.rm = TRUE)+
  facet_grid(~Summit)+
  labs(x="",
       y="",
       color="Alter",size="Cover difference")+
...
pr <- gridExtra::grid.arrange(pr, 
                              ...
                              bottom = textGrob("Temperature niche (°C)", 
                                                vjust = -1,
                                                hjust=0.7),
                              top= textGrob("(b) MC2 to MC3", hjust = 2.5))  
ggsave(paste0(svpath,"/plot/cover_vs_nich_MC23.jpeg"),plot=pr,width=8,height=5,dpi=600)
```
